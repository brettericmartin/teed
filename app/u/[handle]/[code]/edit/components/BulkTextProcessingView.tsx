...
import { Check, X, AlertCircle, Image as ImageIcon } from 'lucide-react';
...
function StreamingItemRow({ index, item, currentStage, stages }: StreamingItemRowProps & { stages: TextProcessingStage[] }) {
        ...
        <div className="flex-1 min-w-0">
          {item.status === 'completed' && item.result && item.result.status !== 'failed' ? (
            ...
            <>
              ...
              <p className="text-xs text-red-500 truncate">{item.rawText}</p>
            </>
          ) : (
            <>
              <p className="text-sm text-gray-700 truncate">{item.rawText}</p>
              {currentStage && (
...
interface BulkTextProcessingViewProps {
  totalItems: number;
  completedCount: number;
  streamingResults: Map<number, TextStreamingItem>;
  currentStages: Map<number, TextProcessingStage>;
  mode?: 'identify' | 'enhance';
}

const IDENTIFY_STAGES: TextProcessingStage[] = ['parsing', 'searching', 'enriching'];
const ENHANCE_STAGES: TextProcessingStage[] = ['linking', 'imaging'];

export function BulkTextProcessingView({
  totalItems,
  completedCount,
  streamingResults,
  currentStages,
  mode = 'identify',
}: BulkTextProcessingViewProps) {
  const isComplete = completedCount >= totalItems && totalItems > 0;
  const stages = mode === 'enhance' ? ENHANCE_STAGES : IDENTIFY_STAGES;
  const actionWord = mode === 'enhance' ? 'enhanced' : 'identified';
    ...
    <div className="flex flex-col h-full min-h-[300px]">
        ...
        <p className="text-sm text-gray-600 mt-3 text-center font-medium">
          {isComplete ? (
            <span className="text-[var(--teed-green-9)]">
              All {totalItems} items {actionWord}!
            </span>
          ) : (
            <>
      ...
      {!isComplete && (
        <div className="p-4 border-t border-gray-100 flex justify-center items-center gap-3">
          <GolfLoader size="md" variant="bounce" label={mode === 'enhance' ? 'Enhancing products...' : 'Identifying products...'} />
          <span className="text-sm text-gray-500">{mode === 'enhance' ? 'Enhancing...' : 'Identifying...'}</span>
        </div>
      )}
    </div>
  );
...
export default BulkTextProcessingView;
