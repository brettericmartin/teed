...
function PlatformIcon({ classification }: { classification: LinkClassification }) {
  const iconClass = "w-6 h-6 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center flex-shrink-0";
  const svgClass = "w-3 h-3 sm:w-4 sm:h-4 fill-white";
  const lucideClass = "w-3 h-3 sm:w-4 sm:h-4 text-white";

  if (classification.type === 'embed') {
    switch (classification.platform) {
      case 'youtube':
        return (
          <div className={`${iconClass} bg-[#FF0000]`}>
      ...
      case 'tiktok':
        return (
          <div className={`${iconClass} bg-black`}>
            <svg viewBox="0 0 24 24" className={svgClass}>
      ...
      case 'spotify':
        return (
          <div className={`${iconClass} bg-[#1DB954]`}>
            <Music className={lucideClass} />
          </div>
        );
      case 'instagram':
        return (
          <div className={`${iconClass} bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#F77737]`}>
            <svg viewBox="0 0 24 24" className={svgClass}>
      ...
      case 'twitch':
        return (
          <div className={`${iconClass} bg-[#9146FF]`}>
            <svg viewBox="0 0 24 24" className={svgClass}>
      ...
      default:
        return (
          <div className={`${iconClass} bg-[var(--teed-green-9)]`}>
            <Video className={lucideClass} />
          </div>
        );
    }
  }
  ...
  if (classification.type === 'social') {
    ...
    switch (classification.platform) {
      ...
      case 'linkedin':
        return (
          <div className={`${iconClass} bg-[#0A66C2]`}>
            <User className={lucideClass} />
          </div>
        );
      case 'github':
        return (
          <div className={`${iconClass} bg-[#24292F]`}>
            <User className={lucideClass} />
          </div>
        );
      case 'facebook':
        return (
          <div className={`${iconClass} bg-[#1877F2]`}>
            <User className={lucideClass} />
          </div>
        );
      case 'tiktok':
      ...
      default:
        return (
          <div className={`${iconClass} bg-[var(--sky-9)]`}>
            <User className={lucideClass} />
          </div>
        );
    }
  ...
  // Product
  return (
    <div className={`${iconClass} bg-[var(--copper-8)]`}>
      <ShoppingBag className={lucideClass} />
    </div>
  );
}
      ...
      {classifiedUrls.length > 0 && (
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {counts.embeds > 0 && (
              <span className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-[var(--teed-green-2)] text-[var(--teed-green-10)] text-xs sm:text-sm font-medium">
                <Video className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                {counts.embeds} embed{counts.embeds !== 1 ? 's' : ''}
              </span>
            )}
            {counts.socials > 0 && (
              <span className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-[var(--sky-2)] text-[var(--sky-10)] text-xs sm:text-sm font-medium">
                <User className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                {counts.socials} profile{counts.socials !== 1 ? 's' : ''}
              </span>
            )}
            {counts.products > 0 && (
              <span className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-0.5 sm:py-1 rounded-full bg-[var(--copper-2)] text-[var(--copper-9)] text-xs sm:text-sm font-medium">
          ...
          <div className="space-y-1.5 sm:space-y-2 max-h-48 sm:max-h-64 overflow-y-auto">
            {classifiedUrls.map(({ url, classification }, i) => (
              <div
                key={i}
                className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl bg-[var(--surface-elevated)] border border-[var(--border-subtle)]"
              >
                <PlatformIcon classification={classification} />
                <div className="flex-1 min-w-0">
                  <p className={`text-[10px] sm:text-xs font-medium ${getTypeColor(classification.type)}`}>
      ...
      {parsedUrls.length === 0 && (
        <div className="flex flex-col items-center justify-center py-4 sm:py-8 text-center">
          <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-[#FF0000] flex items-center justify-center">
            ...
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br from-[#833AB4] via-[#FD1D1D] to-[#F77737] flex items-center justify-center">
              <svg viewBox="0 0 24 24" className="w-4 h-4 sm:w-5 sm:h-5 fill-white">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
              </svg>
            </div>
            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-[var(--copper-8)] flex items-center justify-center">
