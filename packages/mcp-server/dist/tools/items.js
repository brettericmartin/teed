...
exports.handleItemTool = handleItemTool;
 ...
 */
async function handleItemTool(name, args, supabase, userId) {
    switch (name) {
...
async function addItemToBag(supabase, userId, args) {
    if (!userId) {
        return {
    ...
    const purchaseUrl = args?.purchase_url;
    if (purchaseUrl) {
...
async function updateItem(supabase, userId, args) {
    if (!userId) {
        return {
    ...
    const bagOwner = (Array.isArray(bagData) ? bagData[0]?.owner_id : bagData?.owner_id);
    if (bagOwner !== userId) {
        return {
    ...
    const updates = {};
    if (args?.name)
        updates.custom_name = args.name;
    if (args?.brand !== undefined)
        updates.brand = args.brand;
    if (args?.description !== undefined)
        updates.custom_description = args.description;
...
async function removeItemFromBag(supabase, userId, args) {
    if (!userId) {
        return {
...
async function moveItemToBag(supabase, userId, args) {
    if (!userId) {
        return {
    ...
    const itemId = args?.item_id;
    const targetBagCode = args?.target_bag_code;
    ...
    const targetBag = await (0, supabase_js_1.verifyBagOwnership)(supabase, targetBagCode, userId);
    if (!targetBag) {
        return {
...
async function copyItemToBag(supabase, userId, args) {
    if (!userId) {
        return {
    ...
    const sourceBag = sourceItem.bags;
