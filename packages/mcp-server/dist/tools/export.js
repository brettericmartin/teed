...
exports.handleExportTool = handleExportTool;
exports.exportTools = [
    {
 ...
 */
async function handleExportTool(name, args, supabase, userId) {
    ...
    switch (name) {
        case 'export_bag_youtube':
            return exportYouTube(bag, items, bagUrl, displayName, args?.include_affiliate_disclaimer !== false);
        case 'export_bag_newsletter':
            return exportNewsletter(bag, items, bagUrl, displayName);
        case 'export_bag_markdown':
            return exportMarkdown(bag, items, bagUrl, displayName);
        case 'export_bag_text':
            return exportText(bag, items, bagUrl, displayName);
        default:
            return {
...
function exportYouTube(bag, items, bagUrl, displayName, includeDisclaimer) {
    const lines = [];
    // Header
    ...
    items.forEach((item) => {
        ...
        const purchaseLink = item.links?.find((l) => ['purchase', 'affiliate', 'product', 'buy'].includes(l.kind));
        if (purchaseLink) {
            lines.push(`▸ ${fullName}`);
...
function exportNewsletter(bag, items, bagUrl, displayName) {
...
function exportMarkdown(bag, items, bagUrl, displayName) {
    const lines = [];
    lines.push(`# ${bag.title}`);
    lines.push('');
...
function exportText(bag, items, bagUrl, displayName) {
    const lines = [];
    lines.push(bag.title);
