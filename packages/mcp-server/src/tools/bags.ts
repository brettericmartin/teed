...
import { getBagByCode, generateBagCode, verifyBagOwnership } from '../utils/supabase.js';

export const bagTools: Tool[] = [
  {
    name: 'list_my_bags',
...
async function getBag(
  ...
  const bag = await getBagByCode(supabase, bagCode, userId);
  ...
  const items = (bag.bag_items as Array<Record<string, unknown>>).map((item) => {
    const catalogItem = item.catalog_item as Record<string, unknown> | null;
    return {
      id: item.id,
...
async function createBag(
  ...
  let code = generateBagCode(title);
  let suffix = 0;
